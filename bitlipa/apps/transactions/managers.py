...
class TransactionManager(models.Manager):
    def list(self, user=None, **kwargs):
        ...
        if table_fields.get('source_address') and table_fields.get('source_address') == table_fields.get('target_address'):
            ...
            query = query & models.Q(**{'deleted_at': None, **remove_dict_none_values(table_fields)})
        else:
    ...
    def topup_funds(self, user=None, **kwargs):
        ...
        method="POST"
        response = m_pesa_http(url, method, payload, access_token[0].access_token)
        res = response.json()
    ...
    def create_topup_transaction(self, **kwargs):
        ...
        callback_data=data.get('CallbackMetadata') if isinstance(data.get('CallbackMetadata'), dict) else {}
